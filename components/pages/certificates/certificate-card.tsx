        ...
        <CardContent className="mt-auto space-y-4">
          <div className="flex min-h-7 items-start justify-between gap-3">
            ...
            {certificate.icons && certificate.icons.length > 0 && (
              <ul className="flex shrink-0 flex-wrap justify-end gap-1.5" role="list" aria-label="Certificate technologies">
                {certificate.icons.map((icon) => (
                  <li
                    key={icon}
                    className="relative size-7 rounded-md border border-border/50 bg-background/60"
                  >
                    <Image
                      src={icon}
                      alt=""
